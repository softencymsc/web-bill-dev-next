...
function CustomerFormInner() {
  ...
  const id = searchParams?.get('id'); // e.g., "CUS001"
  ...
  useEffect(() => {
    if (!id || !isValidIdFormat(id)) {
      setCodeError('Invalid customer ID format. Expected format: CUS followed by digits (e.g., CUS001)');
    ...
    const fetchCustomerData = async () => {
      ...
      try {
        ...
        const q = query(customersRef, where('CUSTCODE', '==', id));
        const querySnapshot = await getDocs(q);

        if (querySnapshot.empty) {
          setCodeError('No customer found with this ID');
          // toast.error('No customer found with this ID');
          return;
        }
        ...
        setCodeError('Failed to fetch customer data');
  ...
  const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    ...
    const checked = e.target instanceof HTMLInputElement && 'checked' in e.target ? e.target.checked : false;
    setFormData((prev) => ({
      ...prev,
  ...
  const handleUpdate = async () => {
    ...
    if (!docId) {
      // toast.error('No customer document found to update');
      return;
    }
              ...
              <div>
                <div className="flex items-center">
                  <input
                    ...
                    id="acceptTerms"
                    name="acceptTerms"
                    className="h-4 w-4 text-[#2c5aa0] focus:ring-[#2c5aa0] border-gray-300 rounded disabled:bg-gray-50"
                    checked={formData.acceptTerms}
                    onChange={handleChange}
                    disabled={!isEditing}
                  />
                  <label htmlFor="acceptTerms" className="ml-2 text-gray-600 text-sm font-medium">
                    Accept Terms
                  </label>
                </div>
              </div>
